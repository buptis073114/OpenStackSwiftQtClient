#include "swift_delete_buckets.h"

Swift_Delete_Buckets::Swift_Delete_Buckets(QObject *parent) : QObject(parent)
{
    if (NULL == manager) {
...
Swift_Delete_Buckets::Swift_Delete_Buckets(QMap<QString,QString>* token_message,
                                           QString bucketname
...
void Swift_Delete_Buckets::DoSetup(QThread &cThread){
    connect(&cThread, SIGNAL(started()), this, SLOT(slot_swift_delete_bucket()));
}


void Swift_Delete_Buckets::slot_swift_delete_bucket(){
    slot_swift_delete_bucket(this->token_message,this->bucketname);
}

void Swift_Delete_Buckets::slot_swift_delete_bucket(QMap<QString,QString>* token_message,QString bucketname){
...
void Swift_Delete_Buckets::replyFinished(QNetworkReply* reply){
    ...
    emit signal_swift_delete_bucket_success(ret_header_map);
...
void Swift_Delete_Buckets::slot_sslErrors(QNetworkReply *reply, const QList<QSslError> &errors){

    QSslCertificate sslcert = errors[0].certificate();
    reply->ignoreSslErrors();
}

void Swift_Delete_Buckets::provideAuthenication(QNetworkReply* reply, QAuthenticator* authenticator){



}

void Swift_Delete_Buckets::NetWorkError(QNetworkReply::NetworkError){


    emit signal_swift_delete_bucket_error("NetworkError");
    if (NULL != post_reply) {
        //ret_data = post_reply->readAll();
        //qDebug() << Q_FUNC_INFO << "reply readAll is " << ret_data;
        post_reply->deleteLater();
...
Swift_Delete_Buckets::~Swift_Delete_Buckets(){
